...
class EditProductsViewModel : ViewModel() {
    ...
    fun checkFields(id_: String, name_: String, type_: String, price_: String, desc_: String) {
        if(name_.isEmpty() || type_.isEmpty() || price_.isEmpty() || desc_.isEmpty())
            _msg.value = "Todos los campos deben ser llenados"
        else
            viewModelScope.launch(Dispatchers.IO){
                val product = Product(id = id_, productName = name_, productType = type_, productPrice = price_.toInt(), productDescription = desc_)
